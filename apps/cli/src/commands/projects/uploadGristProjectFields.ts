...
import { createProjectRecords, GristProjectFields, listProjectRecords } from "@sde/cli/grist/grist"
...
export const uploadGristProjectFields = new Command()
  ...
  .action(async () => {
    ...
    const existingGristProjectsId = new Set(existingGristProjects.map(project => project.fields.drupal_id))
    
    const gristProjects = JSON.parse(fs.readFileSync(gristOutputFile, 'utf8')) as GristProjectFields[]
    const gristsProjectsToUpload = gristProjects.filter(project => !existingGristProjectsId.has(project.drupal_id))
